"""Create four deterministic branches, score them, and prune losing tips."""
...
def main() -> None:
    ...
    with runtime.run(
        "best-of-n", budget=Budget(tokens=120_000, depth=12, steps=20)
    ) as run:
        payload = {
            "model": "mock-1",
        ...
        for attempt in range(4):
            with run.branch(attempt=attempt) as branch:
                ...
                score = branch.tool_call(
                    "judge", {"text": candidate.result["text"]}, fn=judge
                )
